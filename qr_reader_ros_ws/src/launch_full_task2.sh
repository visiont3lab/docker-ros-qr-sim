...
sleep 15 && docker exec -it ros_melodic_desktop_full_task_2 /bin/bash -c 'source /root/catkin_ws/devel/setup.bash && roslaunch qr_detector_pkg qr_detector_cams.launch --wait';
exec bash\""  \
--tab --title "WEB_VIDEO_SERVER" --command "bash -c \"
sleep 15 && docker exec -it ros_melodic_desktop_full_task_2 /bin/bash -c 'source /root/catkin_ws/devel/setup.bash && roslaunch web_video_server web_video_server.launch --wait';
exec bash\"" & 